//! `SeaORM` Entity, @generated by sea-orm-codegen 1.1.20

use sea_orm::entity::prelude::*;
use serde::{Deserialize, Serialize};

#[derive(Clone, Debug, PartialEq, DeriveEntityModel, Eq, Serialize, Deserialize)]
#[sea_orm(table_name = "open_source_project")]
pub struct Model {
    #[sea_orm(primary_key, auto_increment = false, column_type = "Text")]
    pub id: String,
    #[sea_orm(column_type = "Text")]
    pub internship_id: String,
    #[sea_orm(column_type = "Text")]
    pub platform: String,
    #[sea_orm(column_type = "Text")]
    pub owner: String,
    #[sea_orm(column_type = "Text")]
    pub repo: String,
    #[sea_orm(column_type = "Text")]
    pub repository_url: String,
    #[sea_orm(column_type = "Text", nullable)]
    pub webhook_token: Option<String>,
    #[sea_orm(column_type = "Text", nullable)]
    pub display_name: Option<String>,
    pub created_at: DateTime,
    pub updated_at: DateTime,
}

#[derive(Copy, Clone, Debug, EnumIter, DeriveRelation)]
pub enum Relation {
    #[sea_orm(
        belongs_to = "super::open_source_internship::Entity",
        from = "Column::InternshipId",
        to = "super::open_source_internship::Column::Id",
        on_update = "NoAction",
        on_delete = "Restrict"
    )]
    OpenSourceInternship,
    #[sea_orm(has_many = "super::open_source_project_maintainer::Entity")]
    OpenSourceProjectMaintainer,
}

impl Related<super::open_source_internship::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::OpenSourceInternship.def()
    }
}

impl Related<super::open_source_project_maintainer::Entity> for Entity {
    fn to() -> RelationDef {
        Relation::OpenSourceProjectMaintainer.def()
    }
}

impl ActiveModelBehavior for ActiveModel {}
